...
def generate(name, tmin, tmax, emax, queue, data):
    ...
    '\npython ' + join(abspath(lateasy.__file__), 'run_fermianalysis.py') + ' --fermiconf ' + ymlname + ' --pipeconf ' + args.pipeconf, 
    '\ncp ' + dirname + '.* ' + dirname,
    '\ncd ' + dirname,
    '\npython ' + join(abspath(lateasy.__file__) + 'run_sensitivity.py'),
